...
VSCodeの設定ファイル。このファイルに記述した設定はグローバルの設定を上書きし、このディレクトリ内でのみ有効となる。個人の環境に応じて編集可。プロジェクトをGit管理する場合、追跡対象としなくてよい。
### .gitignore
Gitの追跡対象にしないファイルを記述する。<br>
node_modulesと.DS_Storeは追跡対象としない。必要に応じて追記する。
### .stylelintrc.json
stylelintの設定ファイル。
