...
5. （プロジェクトをGit管理する場合）.gitignoreファイルの```#.vscode```から```#```を削除し.vscodeディレクトリをGitの追跡対象外とする。```git init```コマンドの実行または任意のGUIツールの操作により、Gitを開始する
6. README.mdの「本レポジトリの使用方法」セクションを削除する
7. フォルダ・ファイルを作成し、コーディングを開始する
...
### .stylelintrc.json
stylelintの設定ファイル。使用するルールについては後述のガイドライン内に記述。
### README.md
